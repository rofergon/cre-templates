# Workflow CRE Chainlink - Guia Tecnica (Legacy)

> Legacy note:
> This document describes a pre-refactor workflow version centered on vesting/grants
> (`EmployeeVesting`, `SYNC_CREATE_GRANT`, `GrantCreated`, `GrantRevoked`,
> `TokensClaimed`) and outdated trigger indexing. It has been moved to `Docs/Legacy/`
> because it no longer matches the current protocol implemented in `EquityWorkflowCre/main.ts`.
## 1) Objetivo
